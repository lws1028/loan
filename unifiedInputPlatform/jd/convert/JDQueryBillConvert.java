package com.zlhj.unifiedInputPlatform.unifiedInputPlatform.jd.convert;
...
public class JDQueryBillConvert {

    private final static String REPAYMENT_TYPE = "EQUAL_PRINCIPAL_INTEREST";

    public static JDClueQueryBillVO convert(JDClueQueryBillBusiness business) {
        // 校验基本对象
        ...
        // 还款计划列表
        List<JDClueQueryBillVO.RepaymentPlanDTO> repaymentPlanList = loanRepayDetailObjects.stream()
        ...
        // 封装结果
        JDClueQueryBillVO jdClueQueryBillVO = new JDClueQueryBillVO();
        jdClueQueryBillVO.setMhtIouNo(mhtIouNo);
        jdClueQueryBillVO.setCashAmount(ConvertionUtil.getSimpleStringWithNull(cashAmount));
        jdClueQueryBillVO.setPendingAmount(ConvertionUtil.getSimpleStringWithNull(pendingAmount));
        jdClueQueryBillVO.setTotalPeriod(ConvertionUtil.getSimpleStringWithNull(totalPeriod));
        jdClueQueryBillVO.setRepaymentType(REPAYMENT_TYPE);
        jdClueQueryBillVO.setStatus(status);
        jdClueQueryBillVO.setOverdueStatus(overdueStatus);
        jdClueQueryBillVO.setCashCompleteDate(cashCompleteDate);
        jdClueQueryBillVO.setLoanEndDate(loanEndDate);
        jdClueQueryBillVO.setNormalRate(ConvertionUtil.getSimpleStringWithNull(normalRate));
        jdClueQueryBillVO.setRepaymentPlanList(repaymentPlanList);

        return jdClueQueryBillVO;
    }


    public static JDClueQueryBillVO convertForJdJt(JDClueQueryBillBusiness business) {
        // 校验基本对象
        MultipleLoanObject loanObject = Optional.ofNullable(business.getMultipleLoanObject())
                .orElseThrow(() -> new BusinessException("线索没有匹配的贷款数据"));
        List<LoanRepayDetailObject> loanRepayDetailObjects = Optional.ofNullable(business.getLoanRepayDetailObjects())
        ...
        jdClueQueryBillVO.setNormalRate(ConvertionUtil.getSimpleStringWithNull(normalRate));
        jdClueQueryBillVO.setRepaymentPlanList(repaymentPlanList);
        jdClueQueryBillVO.setRepaymentPendingDate(repaymentPendingDate);
        return jdClueQueryBillVO;
    }
    /**
     * 逾期 状态
    ...
    private static BigDecimal actualPremiumOverdueAmount(AdditionalFeeRepayPlan gpsAdditionalFeeRepayPlan){
        ...
        return JDQueryBillConvert.shouldPremiumRepayAmount(gpsAdditionalFeeRepayPlan).subtract(JDQueryBillConvert.actualPremiumRepaymentAmount(gpsAdditionalFeeRepayPlan));
    }
